...
CSV_FILE = "C:\\Users\\migue\\Documents\\programacion-2025\\prog-2025-2-10am-unidad5-MiguelSCpro\\RETO\\pasajeros_movilizados_mensual_metro_de_medellin.csv"
TXT_FILE = "C:\\Users\\migue\\Documents\\programacion-2025\\prog-2025-2-10am-unidad5-MiguelSCpro\\RETO\\aeronautica.txt"
# ---------------- FUNCIONES PARA TXT ----------------
def contar_txt():
...
def reemplazar_txt():
    ...
    new = input("Palabra nueva: ")
    with open(TXT_FILE, "r", encoding="utf-8", errors="ignore") as f:
        texto = f.read()
    texto = texto.replace(buscar, new)
    with open(TXT_FILE, "w", encoding="utf-8") as f:
        f.write(texto)
    print("Se reemplazó correctamente")
...
def graficar_columna():
    with open(CSV_FILE, "r", encoding="utf-8", errors="ignore") as f:
        ...
        datos = []
        for fila in lector:
            if len(fila) <= columna or fila[columna] == "":
                continue

            if fila[columna] != "":
                try:
                    datos.append(float(fila[columna].replace(",", ".")))
                except:
                    pass
    if len(datos) == 0:
        print("No hay datos numéricos.")
    ...
    plt.show()
    datos.sort()
    plt.bar(range(len(datos)), datos, color="red")
...
def main():
    while True:
        ...
        print("2. Procesar archivo TXT (aeronautica.txt)")
        print("3. Procesar archivo CSV (pasajeros_movilizados_mensual_metro_de_medellin.csv)")
        print("4. Salir")

        opcion = input("Escribe el número de la opción: ")

        if opcion == "1":
            print(os.listdir("./RETO"))
        elif opcion == "2":
            menu_txt()
        elif opcion == "3":
            menu_csv()
